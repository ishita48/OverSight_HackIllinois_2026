// projects/[id]/ProjectOverviewClient.jsx
"use client";
...
export default function ProjectOverviewClient({ project }) {
  const router = useRouter();
  const [copiedSection, setCopiedSection] = useState(null);

  const handleShare = () => {
    // Navigate to public project share page
    router.push(`/u/${project.id}`);
  };

  const handleNewSession = () => {
    const sessionId = generateUUID();
    router.push(`/record/${sessionId}?projectId=${project.id}`);
  };
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        >
          ...
          <div className="space-y-4">
            {project.sessions.map((session, index) => (
              ...
              >
                <Card className="bg-white/90 border-[#F7F5F2] hover:border-[#D4735F]/30 hover:shadow-lg transition-all duration-300 rounded-2xl group">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-start gap-4">
                          <div className="flex-1">
                            <h3 className="text-lg font-serif text-[#2C2825] mb-2 group-hover:text-[#D4735F] transition-colors">
                            ...
                            <div className="flex items-center gap-4 text-sm text-[#6B6560] mb-3">
                              ...
                              {session.mood && (
                                <Badge className="bg-[#D4735F]/10 text-[#D4735F] border-[#D4735F]/20 rounded-full px-2 py-1 text-xs">
                            ...
                            {session.oneLiner && (
                              <div className="bg-[#8B9DC3]/10 rounded-xl p-3 mb-3">
                                <Quote className="w-3 h-3 text-[#8B9DC3] mb-1" />
                                <p className="text-[#2C2825] text-sm italic">
                                  "{session.oneLiner}"
                                </p>
                              </div>
                            )}
        ...
        >
          <Button
            ...
            onClick={handleShare}
            className="border-[#F7F5F2] text-[#6B6560] hover:bg-[#8B9DC3]/10 hover:text-[#8B9DC3] 
                     hover:border-[#8B9DC3] rounded-xl transition-all duration-200"
          >
